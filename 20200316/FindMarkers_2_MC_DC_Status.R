...
saveRDS(markers,file.path(args$outdir,"DC_HER_markers.rds"))
write.table(markers,file.path(args$outdir,"DC_HER_markers.csv"),quote=F,row.names=F,sep=",")

topn=markers%>%group_by(cluster)%>%top_n(25,wt=avg_logFC)
genes<-topn$gene
jpeg(file.path(args$outdir,"DC_HCER_heatmap1.jpeg"),width=1024,height=1024)
DoHeatmap(seurat,features=genes)
dev.off()

jpeg(file.path(args$outdir,"DC_HCER_heatmap2.jpeg"),width=1024,height=1024)
