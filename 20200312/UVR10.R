...
path="/Data/zoc/result/PBMC/10X-count/10X-VDJ/human/5RNA/UVR-10-5RNA/outs"
...
#cluster<-subset(cluster,select=Cluster,Cluster!=11)
cluster<-subset(cluster,select=Cluster,Cluster%in%c(1,2,3,4,5)) # T
#cluster<-subset(cluster,select=Cluster,Cluster%in%c(1,2,3,4,5,9,11)) # NKT
